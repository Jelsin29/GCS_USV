...
class TelemetryWidget(QWidget, Ui_TelemetryWidget):
    def __init__(self, parent=None):
        ...
        QTimer.singleShot(
            100, self.initializeZeroValues
        )  # **CHANGED: Initialize with zero values**
        ...
        if hasattr(self, "titleLabel"):
            if is_simulation:
                ...
                title = (
                    ...
                    else "USV TELEMETRY - DISCONNECTED"
                )
    ...
    def updateFromVRXData(self, vrx_telemetry):
        ...
        try:
            ...
            if "global_position_int" in vrx_telemetry:
                ...
                lat = (
                    gps_data.get("lat", 0) / 1e7
                )  # Convert from int to decimal degrees
        ...
        try:
            ...
            if hasattr(self, "headingValueLabel") and hasattr(self, "headingLabel"):
                ...
                if classification_confidence is not None:
                    perception_text = (
                        f"{objects_detected} ({classification_confidence:.1f}%)"
                    )
                else:
                ...
                if classification_confidence is not None:
                    if classification_confidence > 80:
                        ...
                        color_style = (
                            "color: #f44336; font-weight: bold;"  # Low confidence - red
                        )
                else:
            ...
            print(
                f"Perception: {objects_detected} objects, {classification_confidence}% confidence"
            )
        except Exception as e:
    ...
    def updateHeading(self, heading):
        """Legacy method - heading update (was incorrectly redirecting to roll)"""
        pass
    ...
    def updateDirection(self, heading_degrees):
        ...
        print(
            f"Direction received but not displayed: {heading_degrees:.0f}° ({direction})"
        )
